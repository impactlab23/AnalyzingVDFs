...
t_test_single_sim <- function(n_sample,        #trial sample size
                              F_90_d_trt,    #cumulative hazard at day 90
                              F_90_d_ctl,    ##cumulative hazard at day 90
                              F_28_v_trt,    #cumulative liberation rate from ventilator at day 28
                              F_28_v_ctl,    #cumulative liberation rate from ventilator at day 28
  ...
  tryCatch({
    ...
    t_test <- t.test(VFD_OO_2 ~ treatment, data = data, alternative = "less")
...
Sim_Res <- foreach(k = 1:n_sim, .combine = rbind,
  ...
  set.seed(k + 5125)
  t_test_single_sim(n_sample = 1500,        #trial sample size
...
write.csv(results, file = "C:/Users/Anna Heath/OneDrive/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Weibull_AH/ttest/ttest_imp_mort_worse_lib.csv")
