...
Wilcox_single_sim <- function(n_sample,        #trial sample size
  ...
  data <- data %>% 
    mutate(treatment = as.factor(ifelse(treatment == "trt", 1, 0)),
           VFD_OO_2 = ifelse(VFD_OO_2 == -1, 0, VFD_OO_2))
  #####################################################
  #####    Analysis               #####################
  #####################################################
  tryCatch({
...
write.csv(results, file = "C:/Users/Anna Heath/OneDrive/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Weibull_AH/wilcox/wilcox_imp_mort_worse_lib.csv")
