#############################################################
########### multinomial wilcox imp mort worse lib#############
#############################################################
library(tidyverse)
library(dplyr)
library(boot)
...
Sim_Res <- foreach(k = 1:n_sim, .combine = rbind,
  ...
  set.seed(k + 5125)
  wilcox_single_sim(OR_mort = 0.75, 
                   OR_lib = 1.25)     
}
stopCluster(cl)
...
write.csv(results, file = "C:/Users/Anna Heath/OneDrive/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Multinomial_AH/wilcox/wilcox_imp_mort_worse_lib.csv")
