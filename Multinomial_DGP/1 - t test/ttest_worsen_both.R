#############################################################
########### multinomial t test worsen both     ##############
#############################################################
library(tidyverse)
library(dplyr)
library(boot)
...
Sim_Res <- foreach(k = 1:n_sim, .combine = rbind,
  ...
  set.seed(k + 5125)
  ttest_single_sim(OR = 1.25)     
}
stopCluster(cl)

results <- as.data.frame(Sim_Res)

write.csv(results, file = "C:/Users/Anna Heath/OneDrive/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Multinomial_AH/t test/ttest_worsen_both.csv")
