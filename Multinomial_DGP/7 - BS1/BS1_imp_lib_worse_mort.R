...
multinomial_data_gen_diff_effect <- function(OR_mort, OR_lib){
  ...
  for(j in 1 : nrow(data)){
    if(data$VFDs[j] == -1){
      censor[j] <- 0
      time[j] <- min(data$death_time[j], 28)
      death[j] <- 1
    }
...
BS1_single_sim <- function(OR_mort, OR_lib){
...
write.csv(results, file = "/home/aheath/Ziming_Sim/BS1/BS1_imp_lib_worse_mort.csv")

save.image(file = "/home/aheath/Ziming_Sim/BS1/BS1_imp_lib_worse_mort.Rdata")
