#############################################################
########### multinomial hurdle model worsening both #########
#############################################################
library(tidyverse)
library(dplyr)
library(boot)
...
Sim_Res <- foreach(k = 1:n_sim, .combine = rbind,
  ...
  set.seed(k + 5125)
  PoissonHurdle_single_sim(OR = 1.25)     
}
stopCluster(cl)
...
write.csv(results, file = "C:/Users/Anna Heath/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Multinomial_AH/hurdle/hurdle_worsen_both.csv")
