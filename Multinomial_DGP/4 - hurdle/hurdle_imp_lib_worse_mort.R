######################################################################
########### multinomial hurdle model imp lib worse mort ##############
######################################################################
library(tidyverse)
library(dplyr)
library(boot)
...
Sim_Res <- foreach(k = 1:n_sim, .combine = rbind,
  ...
  set.seed(k + 5125)
  PoissonHurdle_single_sim(OR_mort = 1.25, 
                           OR_lib = 0.75)     
}
stopCluster(cl)
...
write.csv(results, file = "C:/Users/Anna Heath/OneDrive - SickKids/Admin Anna/Hiring and Staff/Ziming Chen 2022/PRACTICAL/SimulationMar20/Multinomial_AH/hurdle/hurdle_imp_lib_worse_mort.csv")
